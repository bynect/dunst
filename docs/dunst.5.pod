...
=item C<highlight_gradient>

The second highlight color of the notification, used to create a gradient together
with C<highlight>. See COLORS for possible values.
Other two values are also accepted: 'none' for no gradient and 'auto'
to compute a slight gradient.
